...
//			});
		...
		{
			var $el = $(e.target);
			var parent =$el.parents('.inline_image');
			var parent_a = $el.parent('a');

			var callback = $.proxy(function()
			{
				$('#redactor_file_alt').val($el.attr('alt'));
				$('#redactor_file_caption').val($(parent).children('.inline_caption').html());
				...
				{
					$('#redactor_file_link').val($(parent_a).attr('href'));
				}
		...
		imageSave: function(el)
		{
			...
			$(parent).children('.inline_caption').html($('#redactor_file_caption').val());
			...
			if (floating === 'left')
			{
				parent.css({ 'float': 'left', 'padding-left': '0px', 'padding-right' : '' });
			}
			else if (floating === 'right')
			{
				parent.css({ 'float': 'right', 'padding-right': '0px', 'padding-left' : '' });
			}
			...
			if (link !== '')
			{
				if (parent_a.length == 0)
				{
					$(el).replaceWith('<a href="' + link + '" target="_blank">' + this.outerHTML(el) + '</a>');
				}
				else
				{
					$(parent_a).attr('href', link);
				}
			}
			else
			{
				if (parent_a.length > 0)
				{
					$(parent_a).replaceWith(this.outerHTML(el));
				}
			}
		...
		_imageSet: function(json, link)
		{
			...
			if (json !== false)
			{
				...
				if (link !== true)
				{
					html = '<div class="inline_image"><img src="' + json.filelink + '" /><span class="inline_caption"></span></div>';
				}
				else
				{
					html = json;
